...
class Aggregator(Node):
    ...
    def process(self, state:State) -> dict:
        ...
        struct_response = structured_llm.invoke(f"Convert the resume to the desired output format: \n {resume}")
        print("Formatted Resume:", struct_response.json())
        state['resume'] = resume
        state['final_resume'] = struct_response.json()
        return {
